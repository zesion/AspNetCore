...
namespace Microsoft.AspNetCore.SignalR.Protocol
{
    ...
    public static class HandshakeProtocol
    {
        ...
        private static JsonEncodedText MinorVersionPropertyNameBytes = JsonEncodedText.Encode(MinorVersionPropertyName);
        ...
        public static bool TryParseResponseMessage(ref ReadOnlySequence<byte> buffer, out HandshakeResponseMessage responseMessage)
        {
            ...
            while (reader.CheckRead())
            {
                if (reader.TokenType == JsonTokenType.PropertyName)
                {
                    ...
                    else if (reader.TextEquals(MinorVersionPropertyNameBytes.EncodedUtf8Bytes))
                    {
